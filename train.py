...
def preprocess(X, nb_genes = 500, return_idx = False):
    """
    ...
    if return_idx:
        return X, adata.var_names.astype(int).tolist()
    return X
...
def run(X,
    ...
    start = time.time()
    embedding, model = train.train_model(X,
              cluster_number,
              Y=Y,
              nb_epochs=nb_epochs,
              lr=lr,
