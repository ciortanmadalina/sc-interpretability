...
class autoencoder(object):
    def __init__(self, dataname, distribution, self_training, dims, cluster_num, t_alpha, alpha, gamma, learning_rate, noise_sd=1.5, init='glorot_uniform', act='relu'):
        ...
        ####
        ####
        ### Gradient self.likelihood_loss, self.kmeans_loss
        #self.likelihood_loss ,  
        self.var_grad = tf.gradients([self.kmeans_loss, self.likelihood_loss
                                     ] , 
                                     [tf.get_default_graph().get_tensor_by_name("encoder_0/kernel:0")])
#         self.var_grad = tf.gradients([self.likelihood_loss
#                                      ] , 
#                                      [self.x])
    ...
    def funetrain(self, X, count_X, size_factor, batch_size, funetrain_epoch, update_epoch, error, Y):
        ...
        for jj in range(X.shape[0]):
            ...
            gr = self.sess.run(
                [self.var_grad],
                feed_dict={
                    self.sf_layer: size_factor[idx],
                    self.x: X[idx],
                    self.x_count: count_X[idx]})
            gradients.append(np.array(gr).mean(3).reshape(-1))
#             gradients.append(np.array(gr))
        self.gradients = np.array(gradients)#.reshape(len(X), -1)
        
#         clusters = interpret_utils.source_to_target_labels(self.Y_pred, Y)[0]
#         gradients = []
...
#                     self.x_count: count_X[idx]})
#             gradients.append(gr)
#         self.cluster_gradients = np.array(gradients).reshape(len(gradients), -1)
#         vars = tf.trainable_variables()
# #         self.the_weights = vars
#         vars_vals = self.sess.run(vars)
#         for var, val in zip(vars, vars_vals):
#             print(var.name, val.shape)
#             if var.name == "encoder_0/kernel:0":
#                 self.the_weights = val
        self.sess.close()
        return self.Y_pred
